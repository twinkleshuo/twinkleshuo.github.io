...
function lodingdatas(){
	...
	if(window.location.hash){
		var hnum=parseInt(window.location.hash.split("#")[1])-1;
		if(hnum<12){
			$("#menu_ul").append('<li ids='+hnum+'><div class="circle"></div><span>'+data[hnum].song+'</span><span>'+data[hnum].singer.name+'</span><span>'+data[hnum].time+'</span></li>');
			upDateSong(hnum,Trq.volume);
		}else{
	...
	window.onhashchange=function(){
		...
		hnum=parseInt(window.location.hash.split("#")[1])-1;
		if(hnum<12){
			$("#menu_ul").append('<li ids='+hnum+'><div class="circle"></div><span>'+data[hnum].song+'</span><span>'+data[hnum].singer.name+'</span><span>'+data[hnum].time+'</span></li>');
			upDateSong(hnum,Trq.volume);
